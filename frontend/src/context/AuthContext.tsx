...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [tenantId, _setTenantId] = useState(() => {
        if (typeof window !== 'undefined') {
            return localStorage.getItem('current_tenant_id') || 'tenant1';
        }
        return 'tenant1';
    });
    ...
    const setTenantId = (id: string) => {
        ...
        // Force reload or refetch might be needed, but react-query should handle it if keys change,
        // OR we might need to invalidate queries.
